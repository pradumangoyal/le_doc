...
def user_directory_path_orignal(instance, filename):
    ...
    return 'brain_stroke/{0}/orignal/{1}'.format(instance.patient.username, filename)

def user_directory_path_clustered(instance, filename):
    # file will be uploaded to MEDIA_ROOT/user_<id>/<filename>
    return 'brain_stroke/{0}/clustered/{1}'.format(instance.patient.username, filename)

# Create your models here.
class BrainStroke(models.Model):
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='brain_stroke')
    ...
    def __str__(self):
        return self.patient.username + 'brain_stroke'